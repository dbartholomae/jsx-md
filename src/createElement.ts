...
function createTextNode(
  ...
  const nodeValue = Array.isArray(content)
    ? content.flat().map(renderElement).join("")
    : content;
  return {
    nodeName,
    nodeValue,
...
export function createElement(
  functionalComponent: (props: Props) => TextNode | string,
  attributes: null
): TextNode;
export function createElement(
  nodeType: string,
...
export function createElement(
  typeOrComponent: string | ((props: Props) => TextNode | string),
  attributes: null,
  ...children: string[]
): TextNode {
